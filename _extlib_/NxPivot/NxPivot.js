	...
	_pUtil.sortDisplayMembers = function(cube, axisFields, axisDisplayMembers, totalPosition)
	{
		try{
			...
			for (var i=0,len=axisFields.length; i<len; i++) {
				...
				if(baseIsEmpty(sortId)==true||coulumId==sortId){
					valueType = (axisFields[i].type == "number" ? "float" : "string");
					sortDs.addColumn(columnName, valueType);
					sortDs.addColumn(columnName+"_org", valueType);	
				}else{
					columnInfo = dataSource.getColumnInfo(sortId);
					valueType = columnInfo.type;
					sortDs.addColumn(columnName, valueType);
			...
			for (var i=0,len=axisDisplayMembers.length; i<len; i++) {
				...
				for (var j=0,len2=memberList.length; j<len2; j++) {
					...
					if(valueTypes[j]=="string"){
						...
						} else if(sortIds[j])
						{
							...
							for(var k=0;k<=j;k++){
									if(memberList[k]!="-")
									{
										findExprString += "&&"+coulumIds[k]+'=="'+memberList[k]+'"'	
									}else
									{
										findExprString += "&&"+coulumIds[k]+'==""'	
									}
							}
							
							nFRow = dataSource.findRowExpr(findExprString);
							if(nFRow!=-1)
							{
								changeColumn = member;
								member = dataSource.getColumn(nFRow, sortIds[j]);
							}
						}
					}else
					{
					...
					sortDs.setColumn(i, "Column"+j+"_org", changeColumn);
				}
			}
			...
			if ( !NxPivotUtil.isEmpty(axisDisplayMembers) && sortDs.getRowCount() == 0 ) {
				...
				var colCount = sortDs.getColCount()/2;
				for (var i=0,len=sortDs.getRowCount(); i<len; i++) {
					...
					for (var j=0; j<colCount; j++) {
						...
						if(valueTypes[j]=="string"){
							if(sortIds[j])
							{
								changeColumn = sortDs.getColumn(i, "Column"+j+"_org");
								
								if(changeColumn)sortMember[j] = changeColumn;
