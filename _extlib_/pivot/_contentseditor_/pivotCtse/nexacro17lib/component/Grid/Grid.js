...
if (!nexacro.Grid) {
	...
	_pGrid._adjustGridScrollRows_callback_end = function () {
		...
		this._adjustOverlayControls(false, this._is_use_virtualmerge, null, pos);
	};
	...
	_pGrid._adjustGridScrollCols_callback = function () {
		...
		this._adjustOverlayControls(false, this._is_use_virtualmerge, pos);
	};
	...
	_pGrid._autosizeMergeCell = function () {
		...
		if (this.autosizingtype == "both" || this.autosizingtype == "col") {
			for (var i = 0; i < infos.length; i++) {
				...
				var width = this._overlay_controls[i]._adjust_width;
				...
				if (padd) {
					...
					size[0] += (padd.left + padd.right);
				}
				if (bord) {
					bord = new nexacro._BorderObject(bord);
					size[0] += (bord.left._width + bord.right._width);
				}
			...
			for (var i = 0; i < infos.length; i++) {
				...
				if (padd) {
					padd = new nexacro._PaddingObject(padd);
					size[1] += (padd.top + padd.bottom);
				}
				if (bord) {
					bord = new nexacro._BorderObject(bord);
					size[1] += (bord.top._width + bord.bottom._width);
				}
	...
	_pGrid._getCellStyleInfo = function (cellidx, prop, datarow, selected) {
		...
		if (prop == "background") {
			...
			if (!obj) {
				...
				if (text_elem) {
					obj = text_elem._getComputedStyle("text-decoration");
				}
			}
		}
		else {
	...
	_pGrid._hideEditorMergeCell = function (rec) {
		if (!this._tempmergeeditor) {
			return;
		}
		...
		if (!rec && this._isDownActionKeyMouse()) {
			async = true;
		}

		if (!async) {
			var band = this._tempmergeeditor;
			...
			nexacro._OnceCallbackTimer.callonce(this, function () {
				this._hideEditorMergeCell(true);
			});
		}
	};
	...
	_pGrid._setOverlayControlProperty = function (control, left, top, width, height, style_cells, display_text, is_virtual_merge, target_datarow, target_cellinfo, is_create_ctrl) {
		...
		if (style_cells && style_cells.length > 0) {
			...
			var font = this._getCellStyleInfo(cellinfo._cellidx, "font", datarow, false);
			var color = this._getCellStyleInfo(cellinfo._cellidx, "color", datarow, false);
			var letterspace = this._getCellStyleInfo(cellinfo._cellidx, "letterspace", datarow, false);
			var padding = this._getCellStyleInfo(cellinfo._cellidx, "padding", datarow, false);
			var cursor = this._getCellStyleInfo(cellinfo._cellidx, "cursor", datarow, false);
			var wordwrap = this._getCellStyleInfo(cellinfo._cellidx, "wordWrap", datarow, false);

			control.set_font(font);
			control.set_letterSpacing(letterspace);
			control.set_color(color);
			control.set_cursor(cursor);
			control.set_padding(padding);
			control.set_wordWrap(wordwrap);

			var align = this._getCellStyleInfo(cellinfo._cellidx, "align", datarow, false);
	...
	_pGrid._adjustOverlayControls = function (is_create, virtual_merge, hpos, vpos) {
		if (is_create) {
			this._destroyOverlayControls();
		}
		...
		if (this._is_use_virtualmerge && virtual_mergecell_arr.length) {
			...
			for (var i = 0, n = virtual_mergecell_arr.length; i < n; i++) {
				...
				virtual_mergecell = null, e_cell = null;
			}
