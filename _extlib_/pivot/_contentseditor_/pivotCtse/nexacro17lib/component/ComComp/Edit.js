...
if (!nexacro.Edit) {
	...
	_pEdit._beforeinput_process_with_HTMLEvent = function (value, status, begin, end, inputType) {
		...
		var paste_value = update_value ? update_value.replace(/\r\n/g, "") : update_value;
		var paste_value_len = paste_value ? paste_value.length : 0;
		...
		if (inputType == "insertFromPaste") {
			if (update_value != input_value) {
				var maxlength = this.maxlength;
				total_value = input_value.substring(0, begin) + update_value.replace(/\r\n/g, "") + input_value.substring(end);
				if (maxlength && total_value.length > maxlength) {
					total_value = total_value.substring(0, maxlength);
				}
				input_elem._beforeinput_result_data = total_value;
				input_elem._beforeinput_result_pos = {
					begin : begin + paste_value_len, 
					end : begin + paste_value_len
				};
		...
		if (this.maxlength > 0 && (status == 1 || status == 2)) {
			...
			total_value = input_value + paste_value;
