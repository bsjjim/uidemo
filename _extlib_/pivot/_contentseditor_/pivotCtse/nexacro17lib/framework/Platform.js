...
if (!nexacro._bInitPlatform) {
	...
	nexacro.System.print = function (comp, defaultprint, valign, halign, fitonepage) {
		try {
			if (comp) {
				return nexacro._beforePrintCheckPlugin(comp, comp._refform, defaultprint, valign, halign, fitonepage);
			}
		}
		catch (e) {
			return false;
		...
		if (dragInfo && dragInfo.targetwin == this) {
			...
			if (!dragInfo.isDragging) {
				if (windowX != dragInfo.startX || windowY != dragInfo.startY) {
					...
					else if (elem == evt_elem && nexacro._use_offset && offsetX != undefined && offsetY != undefined) {
						canvasX = offsetX;
						canvasY = offsetY;
					}
					else {
